        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {services.map((service, idx) => (
            <motion.div
              ...
              whileHover={{ y: -4 }}
              className="relative p-7 rounded-xl bg-white dark:bg-slate-900 border border-border shadow-sm hover:shadow-md hover:border-green-200 dark:hover:border-slate-700 transition-all duration-200 group cursor-default overflow-hidden"
              data-testid={`card-service-${idx}`}
            >
              {/* Subtle gradient corner */}
              <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-green-50 dark:from-green-950/40 to-transparent rounded-bl-3xl pointer-events-none" />
          ...
          <motion.a
            ...
            whileHover={{ y: -4 }}
            className="relative p-7 rounded-xl border-2 border-dashed border-border hover:border-primary dark:hover:border-primary hover:bg-green-50/50 dark:hover:bg-green-950/20 transition-all duration-200 flex flex-col items-start justify-between group"
