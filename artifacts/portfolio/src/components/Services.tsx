        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {services.map((service, idx) => (
            <motion.div
              ...
              whileHover={{ y: -4 }}
              className="relative p-7 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-blue-200 dark:hover:border-slate-700 transition-all duration-200 group cursor-default overflow-hidden"
              data-testid={`card-service-${idx}`}
            >
              {/* Subtle gradient corner */}
              <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-blue-50 dark:from-blue-950/40 to-transparent rounded-bl-3xl pointer-events-none" />
          ...
          <motion.a
            ...
            whileHover={{ y: -4 }}
            className="relative p-7 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-700 hover:border-primary dark:hover:border-primary hover:bg-blue-50/50 dark:hover:bg-blue-950/20 transition-all duration-200 flex flex-col items-start justify-between group"
